...
shinyServer(function(input, output, session) {
    ...
    output$uinPCs <- renderUI({
        ...
        myValidation(inputData)
        ## print("---------------------")
        showNotification("Rendering UI ...");
        sliderInput(
            "npcs",
            "Number of components",
            min = 1,
            max = min(10, max(2, ceiling(length(inputData$numericCol)/2))),
            value = 3,
            step = 1
        )
    })
    ...
    output$uipx <- renderUI({
        ...
        myValidation(inputData)
        showNotification("Rendering UI ...");
        vals <- setdiff(inputData$nonNumeric, input$rowColor)
        selectizeInput(
            "miscx",
            "X variable: ",
            multiple = F, 
    ...
    runAnalysis <- function(plot=T, type='pca', para=F, both=F){
        ...
        myValidation(inputData)
        uiValidation()
        if( ((!is.nothing(input$npcs) && type=="pca") || 
             ...
             (type=="som" && !is.nothing(input$ydim) && !is.nothing(input$xdim))) &&
            ...
            if(type == "pca"){
                ...
                withProgress(value = 0,
                        ...
                        withProgress(value = 0,
                                     message = "Performing K-mean clusterings analysis",
                                     detail = "...",
                                     {
                                         if(plot){
                                             mat <- apply(inputData$mat, 2, function(x){scale(x, scale=input$scale, center=input$center)})
                                             ...
                                             rownames(mat) <- rownames(inputData$mat)
                                             if(para){ ## stat
                                                 setProgress(value = 0.5, detail = "statistics of K-mean clustering analysis")
                                                 ## 
                                                 m0 <- NULL
                                                 if(input$rowColor != ""){
                             ...
                             detail = "...",
                             {
                                 ...
                                 if(para){
                                     ...
                                     if(both){
                                         ...
                                         setProgress(value = 0.25, detail = "calculating correlation of observations ...")
                                         ...
                                         if(plot){
                                             ...
                                             if(input$circle){
                                                 setProgress(value = 0.75, detail = "generating dendrogram of observations ...") 
                                                 phtm <- pheatmap(cors, plot=F)
                                                 phylo_data <- as.phylo(phtm$tree_row)
                                                 plot(phylo_data, type="fan", tip.color=pcols, 
                                                      main=paste0("Dendrogram of observations, n=", nrow(cors)))
                                             }else{
                                                 setProgress(value = 0.75, detail = "generating heatmap of observations ...") 
                                                 pheatmap(cors, col=mycol, border_color=NA, annotation_row=ann_col_row, 
                                                          annotation_col=ann_col_row, annotation_colors=ann_colors, 
                                                          show_colnames=showlbs, show_rownames=showlbs, 
