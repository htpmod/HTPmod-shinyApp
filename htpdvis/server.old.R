...
shinyServer(function(input, output, session) {
    ...
    prepareData <- function(data, okey){
        if(file.exists(paste0(mydir, "/key.RData")) && file.exists(paste0(mydir, "/.RData"))){
            load(paste0(mydir, "/key.RData"))
            if(okey == key){
                showNotification("load data ...", type='error', duration=30)
    ...
    inputDataAnalysis <- reactive({
        if (input$fileToUse == "demo") {
            ...
            key <- experment <- input$example_data
            inputData <- NULL
            withProgress(value = 0,
            ...
            data0 <- prepareData(inputData, key)
            return(data0); 
        }
    ...
    getFeatureCols <- function(){
        features <- inputDataAnalysis()$numericCol
    ...
    output$uinPCs <- renderUI({
        myValidation()        
        sliderInput(
            "npcs",
            "Number of components",
            min = 1,
            max = min(10, max(2, ceiling(length(inputDataAnalysis()$numericCol)/2))),
            value = 3,
            step = 1
        )
    })

    output$uinDims <- renderUI({
        myValidation()
        inputData <- inputDataAnalysis()
        sliderInput(
            "ndims",
            "Dimension",
            min = 2,
            max = min(ceiling(sqrt(nrow(inputData$mat))), 6),
            value = 3,
            step = 1
        )
    })
    
    output$uinMDS <- renderUI({
        myValidation()
        inputData <- inputDataAnalysis()
        sliderInput(
            "nmds",
    ...
    output$uinCluster <- renderUI({
        myValidation()
        inputData <- inputDataAnalysis()
        sz <- inputData$nonNumericSize
        val <- 3
        # if(input$rowColor %in% names(sz)){
        #     val <- sz[input$rowColor]
        # }
        sliderInput(
            "ncluster",
            "Number of clusters",
            min = 2,
            max = max(ceiling(sqrt(nrow(inputData$mat))), sz),
            value = val,
            step = 1
        )
    })
    
    output$uipx <- renderUI({
        myValidation()        
        vals <- setdiff(inputDataAnalysis()$nonNumeric, input$rowColor)
        selectizeInput(
            "miscx",
            "X variable: ",
            multiple = F, 
    ...
    output$uipy <- renderUI({
        myValidation()        
        vals <- inputDataAnalysis()$numericCol
        selectizeInput(
            "miscy",
            "Y variable: ",
            multiple = F, 
    ...
    output$uinColCluster <- renderUI({
        ...
        sliderInput(
            ...
            "#clusters of columns",
            min = 1,
            max = ceiling(sqrt(ncol(inputData$mat))),
            value = 1,
            step = 1
        )
    })
    
    output$uinSOMx <- renderUI({
        myValidation()
        sz <- inputDataAnalysis()$nonNumericSize
        if(is.null(sz)){
            sz <- 6
        }else{
            sz <- max(sz)
    ...
    runAnalysis <- function(plot=T, type='pca', para=F, both=F){
        ...
        yDim <- input$ydim
        inputData <- inputDataAnalysis()
        if( ((!is.nothing(nPCs) && type=="pca") || 
             (!is.nothing(input$cutree_rows) && !is.nothing(input$cutree_cols) && type=="hca") || 
             ...
             (!is.nothing(input$ncluster) && type=="kmc") || 
             (!is.nothing(input$ndims) && type=="tsne") || 
             (!is.nothing(input$nmds) && type=="mds") || 
             (type=="som" && !is.nothing(yDim) && !is.nothing(xDim))) &&
            ...
            if(type == "pca"){
                ...
                if(plot){
                    if(para){
                        ...
                        pheatmap(pltd, border_color=NA, show_colnames=FALSE, cluster_rows=FALSE, 
                                 ...
                                 ylab="Loading score of features", 
                                 main="\n\n\n\n\n\n\n\n\n\n\n")
                        par(fig=c(0, 1, 0.5, 1), new=TRUE)
                        
                        cols <- rep("grey80", length(R2))
                        cols[1:nPCs] <- 'red'
                        plot(R2, type='h', lwd=2, col=cols, xlab="PC", ylab="% variance", 
                             main="% variance explained by each PC")
                        points(1:length(R2), R2, col=cols, type='b')
                    }else{
                        loading.pch <- 1
                        ...
                        if(input$show_component_score){
                            my.legend("top", xpd=TRUE, legend=names(pchs), ncol=ncol, pch=pchs, col="grey80", text.col='grey80', title=input$rowPoint)
                            my.legend("topright", xpd=TRUE, legend=names(cols), ncol=ncol, col=cols, text.col=cols, pch=16, title=input$rowColor)
                        }
                        if(input$show_factor_loading){
                            my.legend("right", xpd=TRUE, legend=names(gcols), col=gcols, text.col=gcols, pch=NA, lty=1, title="Feature")
                        }
                    }
                }else{
                ...
                if(plot){
                    withProgress(value = 1,
                                 ...
                                 detail = paste("N =", ndims),
                                 {
                                     ## set.seed(123)
                                     tsne_out <- Rtsne(inputData$mat, dims=ndims, perplexity=30, check_duplicates=F, verbose=F, max_iter=1000, 
                                                       pca_center=input$center, pca_scale=input$scale)
                                     colnames(tsne_out$Y) <- paste0("t-SNE", 1:ncol(tsne_out$Y)) 
                                 })
                    withProgress(value = 1,
                                 message = "Making t-SNE graph",
                                 detail = "...",
                                 {
                                     pltd <- tsne_out$Y
                                     if(para){
                                         if(ncol(pltd) > 2){
                                             scatter3D(pltd[,1], pltd[,2], pltd[,3], col=pcols, pch=ppchs, colkey=F, xlab="t-SNE1", ylab="t-SNE2", zlab="t-SNE3", main="3D t-SNE")
                                             my.legend("bottomleft", xpd=TRUE, legend=names(pchs), ncol=1, pch=pchs, col="grey80", text.col='grey80', title=input$rowPoint)
                                             my.legend("bottomright", xpd=TRUE, legend=names(cols), ncol=1, col=cols, text.col=cols, pch=16, title=input$rowColor)
                                         }else{
                                             msgPlot(msg = "Figure\nLegend")
                                             my.legend("topleft", xpd=TRUE, legend=names(pchs), ncol=1, pch=pchs, col="grey80", text.col='grey80', title=input$rowPoint)
                                             my.legend("topright", xpd=TRUE, legend=names(cols), ncol=1, col=cols, text.col=cols, pch=16, title=input$rowColor)
                                         }
                                     }else{
                                         if(ncol(pltd) > 2){
                                             pairs(pltd, col=pcols, pch=ppchs, main="t-SNE", upper.panel=NULL)
                                             my.legend("topright", xpd=TRUE, legend=names(pchs), ncol=1, pch=pchs, col="grey80", text.col='grey80', title=input$rowPoint)
                                             my.legend("right", xpd=TRUE, legend=names(cols), ncol=1, col=cols, text.col=cols, pch=16, title=input$rowColor)
                                         }else{
                ...
                if(plot){
                    ...
                    withProgress(value = 1,
                                 message = "Making MDS graph",
                                 detail = "...",
                                 {
                                     if(para){
                                         if(ncol(pltd) > 2){
                                             scatter3D(pltd[,1], pltd[,2], pltd[,3], col=pcols, pch=ppchs, colkey=F, xlab="MDS1", ylab="MDS2", zlab="MDS3", main="3D MDS")
                                             my.legend("bottomleft", xpd=TRUE, legend=names(pchs), ncol=1, pch=pchs, col="grey80", text.col='grey80', title=input$rowPoint)
                                             my.legend("bottomright", xpd=TRUE, legend=names(cols), ncol=1, col=cols, text.col=cols, pch=16, title=input$rowColor)
                                         }else{
                                             msgPlot(msg = "Figure\nLegend")
                ...
                if(plot){
                    ...
                    withProgress(value = 1,
                                 message = "Performing SOM",
                                 detail = paste0("x-dim=", xDim, ", y-dim=", yDim),
                                 {
                                     mat <- apply(inputData$mat, 2, function(x){scale(x, scale=input$scale, center=input$center)})
                    ...
                    if(para){ ## SOM + PCA 
                        ...
                        clusters <- sort(unique(cluster.labs))
                        ...
                        my.legend("left", xpd=TRUE, legend=names(pchs), ncol=1, pch=pchs, col="grey80", text.col='grey80', title=input$rowPoint)
                        my.legend("right", xpd=TRUE, legend=names(cols), ncol=1, col=cols, text.col=cols, pch=16, title=input$rowColor)
                ...
                if(plot){
                    ...
                    if(para){ ## stat
                        withProgress(value = 1,
                                     message = "Statistics of K-mean clustering analysis",
                                     detail = "...",
                                     {
                                         m0 <- NULL
                                         if(input$rowColor != ""){
                        ...
                        withProgress(value = 1,
                                     message = "Performing K-mean clustering analysis",
                                     detail = "...",
                                     {
                                         res.km <- eclust(mat, "kmeans", k=input$ncluster, graph=F, verbose=F) 
                ...
                if(para){
                    withProgress(value = 1,
                                 message = "Calculating correlation of features",
                                 detail = "...",
                                 {
                    ...
                    if(plot){
                        if(ncol(mat) > 2000){
                            showNotification("Number of features > 2000. Stop here.", type='warning');
                            return(NULL)
                        }
                        gg <- paste0(group, "")
                        ...
                        ## print(factor(paste0(group, "")))
                        withProgress(value = 1,
                                     message = "Generating heatmap of features",
                                     detail = "...",
                                     {
                    ...
                    if(both){
                        if(plot){
                            ...
                            withProgress(value = 1,
                                         message = "Generating heatmap of the whole dataset",
                                         detail = "...",
                                         {
                        ...
                        if(nrow(mat) > 10000){
                            showNotification("Number of observations > 10000. Stop here.", type='warning');
                            return(NULL)
                        }
                        withProgress(value = 1,
                                     message = "Calculating correlation of observations",
                                     detail = "...",
                                     {
                        ...
                        if(plot){
                            ...
                            withProgress(value = 1,
                                         message = "Generating heatmap of observations",
                                         detail = "...",
                                         {
                                             ...
                                             if(input$circle){
                                                 ...
                                                 plot(phylo_data, type="fan", tip.color=pcols, 
                                                      main=paste0("Dendrogram of observations, n=", nrow(cors)))
                                             }else{
                                                 pheatmap(cors, col=mycol, border_color=NA, annotation_row=ann_col_row, 
                                                          annotation_col=ann_col_row, annotation_colors=ann_colors, 
                                                          show_colnames=showlbs, show_rownames=showlbs, 
                                                          main=paste0("Heatmap of observations (rows), n=", nrow(cors)))
                                             }
                                         })
                        }else{
                            cors
            ...
            if(plot){
                ...
                return(list(R2=NULL, loading=NULL))
            }
        }
    }
